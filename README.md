...
(kilroy-face-discord) $ poetry install --extras dev
(kilroy-face-discord) $ kilroy-face-discord
```
...
To build the docs,
`cd` into `kilroy_face_discord/docs` and run:
...
```sh
 docker build -t kilroy-face-discord .
```

To also run the container in one go, run:

```sh
docker build -t kilroy-face-discord . && docker run --rm -it kilroy-face-discord
```
